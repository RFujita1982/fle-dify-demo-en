...
# ============================================================
BRAND = {
    "name": "FLE Global Inc.",               # Page title / browser tab
    "tagline": "Product Manual AI Support",  # Subtitle under engine label
    "engine": "Document AI Engine - α ver.", # Engine name / version (right under header)
    "logo_emoji": "✦",                      # Fallback symbol if no logo image
    "logo_url": "",                          # If set, takes priority over the symbol
    "primary": "#1f3a5f",                    # Main brand color (deep navy)
    "accent": "#c0703a",                     # Accent (terracotta)
    "welcome": "Ask me anything — I'll answer based on the product manuals.",
...
def upload_file_to_dify(uploaded_file):
    ...
    try:
        ...
        st.error(f"⚠️ Upload failed: {uploaded_file.name} ({e})")
        return None
        ...
        with requests.post(url, headers=headers, json=payload,
            ...
            for raw in resp.iter_lines(decode_unicode=True):
                ...
                if event in ("message", "agent_message"):
                    ...
                    yield f"\n\n⚠️ An error occurred: {data.get('message', 'unknown')}"
    except requests.exceptions.RequestException as e:
...
def render_sources(sources: list):
    ...
    if not BRAND["show_sources"] or not sources:
        return
    with st.expander(f"{BRAND['sources_label']} ({len(sources)})"):
        for s in sources:
...
if "user_id" not in st.session_state:
    # Unique user ID (used to distinguish demo sessions in Dify)
    st.session_state.user_id = f"demo-{uuid.uuid4().hex[:8]}"

# APIキー未設定の警告(デモ前のセルフチェック用)
if not DIFY_KEY:
    st.warning("API key is not configured. Please check .streamlit/secrets.toml.")
...
if prompt_value:
    ...
    # ★ 添付ファイルがあったら前の会話を持ち越さない
    # 用途は「会話」ではなく「資料1点ごとに独立した判断」なので、文脈の混在を避ける
    file_attached_starts_new = bool(user_files) and (
        st.session_state.conversation_id != "" or len(st.session_state.messages) > 0
    )
    if file_attached_starts_new:
        st.session_state.conversation_id = ""
        st.session_state.messages = []
    ...
    file_refs = []
    if user_files:
        with st.spinner(f"📎 Uploading files… ({len(user_files)})"):
            for f in user_files:
                ref = upload_file_to_dify(f)
                if ref:
                    file_refs.append(ref)

    # 添付で新規会話を開始した場合は、ユーザーに明示する
    if file_attached_starts_new:
        st.info('📎 New document received. Previous conversation cleared — starting fresh on this document.')
    ...
    if user_text or file_refs:
        ...
        query_for_api = user_text or "Please review the attached file(s) and summarize the key points."
